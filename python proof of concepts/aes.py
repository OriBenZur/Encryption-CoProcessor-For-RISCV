...
A4 = [(x & 0xFF) | ((x & 0xFF) << 8) | ((x & 0xFF) << 16) | ((x & 0xFF) << 24) for x in s_box]

te0, te1, te2, te3, te4 = [], [], [], [], []
for i, x in enumerate(A0):
    te0.append(A0[i].to_bytes(4, byteorder='little'))
    te1.append(A1[i].to_bytes(4, byteorder='little'))
    te2.append(A2[i].to_bytes(4, byteorder='little'))
    te3.append(A3[i].to_bytes(4, byteorder='little'))
    te4.append(A4[i].to_bytes(4, byteorder='little'))
...
class AES:
    ...
    def encrypt_block(self, plaintext):
        ...
        temp_state = sum(plain_state, [])
        plain_state = temp_state
        for i in range(1, self.n_rounds):
            temp_state = plain_state.copy()
            plain_state[0:4] = int.to_bytes(A0[temp_state[0]]^A1[temp_state[5]]^A2[temp_state[10]]^A3[temp_state[15]]^int.from_bytes(self._key_matrices[i][0], 'big'), 4, 'big')
            plain_state[4:8] = int.to_bytes(A0[temp_state[4]]^A1[temp_state[9]]^A2[temp_state[14]]^A3[temp_state[3]]^int.from_bytes(self._key_matrices[i][1], 'big'), 4, 'big')
            plain_state[8:12] = int.to_bytes(A0[temp_state[8]]^A1[temp_state[13]]^A2[temp_state[2]]^A3[temp_state[7]]^int.from_bytes(self._key_matrices[i][2], 'big'), 4, 'big')
            plain_state[12:16] = int.to_bytes(A0[temp_state[12]]^A1[temp_state[1]]^A2[temp_state[6]]^A3[temp_state[11]]^int.from_bytes(self._key_matrices[i][3], 'big'), 4, 'big')

        temp_state = plain_state.copy()
        plain_state[0:4] = int.to_bytes(((s_box[temp_state[0]] << 24) | (s_box[temp_state[5]] << 16) | (s_box[temp_state[10]] << 8) | s_box[temp_state[15]])^int.from_bytes(self._key_matrices[-1][0], 'big'), 4, 'big')
        plain_state[4:8] = int.to_bytes(((s_box[temp_state[4]] << 24) | (s_box[temp_state[9]] << 16) | (s_box[temp_state[14]] << 8) | s_box[temp_state[3]])^int.from_bytes(self._key_matrices[-1][1], 'big'), 4, 'big')
        plain_state[8:12] = int.to_bytes(((s_box[temp_state[8]] << 24) | (s_box[temp_state[13]] << 16) | (s_box[temp_state[2]] << 8) | s_box[temp_state[7]])^int.from_bytes(self._key_matrices[-1][2], 'big'), 4, 'big')
        plain_state[12:16] = int.to_bytes(((s_box[temp_state[12]] << 24) | (s_box[temp_state[1]] << 16) | (s_box[temp_state[6]] << 8) | s_box[temp_state[11]])^int.from_bytes(self._key_matrices[-1][3], 'big'), 4, 'big')

        return bytes(plain_state)
